...
struct PoseEstimator::Impl {
    ...
    Eigen::Vector3d base_to_camera_translation { Eigen::Vector3d::Zero() };
    Eigen::Quaterniond base_to_camera_orientation { Eigen::Quaterniond::Identity() };
    ...
    auto set_base_to_camera_transform(Transform const& transform) -> void {
        transform.position.copy_to(base_to_camera_translation);
        transform.orientation.copy_to(base_to_camera_orientation);
    }

    // actually it will transform the armor from camera frame to odom frame
    ...
    // it will transform the armor from camera frame to base frame
    auto base_to_camera(Armor3D const& armor) const -> Armor3D {
        auto transformed = armor;

        auto position = Eigen::Vector3d { };
        transformed.translation.copy_to(position);
        transformed.translation =
            base_to_camera_orientation * position + base_to_camera_translation;

        auto quat = Eigen::Quaterniond { };
        transformed.orientation.copy_to(quat);
        transformed.orientation = base_to_camera_orientation * quat;

        return transformed;
    }

    auto base_to_camera(std::span<Armor3D const> armors) const -> std::vector<Armor3D> {
        auto result = std::vector<Armor3D> { };
        result.reserve(armors.size());
...
auto PoseEstimator::set_base_to_camera_transform(Transform const& transform) -> void {
    return pimpl->set_base_to_camera_transform(transform);
}
...
auto PoseEstimator::base_to_camera(std::span<Armor3D const> armors) const -> std::vector<Armor3D> {
    return pimpl->base_to_camera(armors);
}
auto PoseEstimator::base_to_camera(Armor3D const& armor) const -> Armor3D {
    return pimpl->base_to_camera(armor);
}
