...
public:
    explicit AutoAimComponent() noexcept
        ...
        register_output("/gimbal/auto_aim/available", gimbal_takeover, false);
        register_output(
    ...
    auto update() -> void override {
        ...
        }
        {
            if (feishu.updated()) {
                ...
                // timestamp check
                if ((now_time - auto_aim_state.timestamp) < std::chrono::milliseconds(0)
                    || (now_time - auto_aim_state.timestamp) > std::chrono::milliseconds(100))
                    return;

                last_valid_aim_time = now_time;

                *shoot_permitted = auto_aim_state.shoot_permitted;
                *gimbal_takeover = true;
                update_target_direction();

            } else if (Clock::now() - last_valid_aim_time > std::chrono::milliseconds(200)) {
                reset_control_commands();
                return;
            }
        }

        *debug_aim_x = target_direction->x();
        *debug_aim_y = target_direction->y();
        *debug_aim_z = target_direction->z();
    }
    ...
    auto update_control_state() -> void {
        ...
        auto odom_to_muzzle_transform =
            fast_tf::lookup_transform<rmcs_description::OdomImu, rmcs_description::MuzzleLink>(
                *rmcs_tf);
        control_state.odom_to_muzzle_transform.position = odom_to_muzzle_transform.translation();
        control_state.odom_to_muzzle_transform.orientation =
            Eigen::Quaterniond(odom_to_muzzle_transform.rotation());
